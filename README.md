...
| Game Mode | **Fn+G** или Options **/**. Зоны в `features/game_lighting.h`. Включает Win-lock и NKRO, **не** SOCD |
| Пресеты света | ISO **Fn+<>** / ANSI **Fn+]**: день → сумерки → ночь → гейминг. Профиль и яркость каждого — EEPROM спустя ~2.5 с |
| Ночник | **Fn+\\** — отдельный зелёный слой. Энкодер: яркость; нажатие — вежливо выкл / вкл. Тот же вид после 10 с USB suspend. Пока режим включён — idle не гасит |
| RGB | **Fn+↑/↓** яркость **текущего** пресета (в ночнике — яркость ночника). Цвет — только C-слой |
...
**Пресеты (C-слой):** день белый 100% / сумерки янтарь 50% / ночь зелёный 15% / гейминг (blackout + зоны). У каждого своя last-used яркость **Fn+↑/↓**. Смена — вспышка целевым цветом, затем fade 3 с (в гейминг 0.5 с). Sleep: 10 / 5 / 10 / 10 мин, fade ~2 с. Ночник (**Fn+\\** или suspend) idle не гасит.
...
USB suspend (крышка ноута / сон хоста): после **10 с** непрерывного suspend лента **переходит в ночник** (~2 с fade), не в чёрный. Пробуждение хоста возвращает пресет, если ночник не зажат **Fn+\\**. Короткий Linux autosuspend (~2 с) подсветку не трогает. Пин A5 драйвера LED не опускаем. `RGB_DISABLE_WHEN_USB_SUSPENDED` специально не включён. Энкодер тоже сбрасывает idle.
